...
class Essai extends Component {
  ...
  saveData = () => {
    ...
    if (this.isContentNull()) {
      ...
      const db = firestore;
      /* besoin creation document cours avec un array vide
      'slides' quand le formateur clique sur ajouter un cours'.
      On a besoin aussi de la clé du parcours et de la clé du cours */
      // const slideSet = db.collection('parcours').doc(localStorage.getItem('id')).
      // collection('cours').add({ slides: [content] });
